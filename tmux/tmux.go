...
func CreateAndSwitchSession(sessionName, worktreePath string, windowCommands []string, command string) error {
	...
	// If a command is provided, send it to opencode and don't switch/attach to the session
	if command != "" {
		sendCmd := exec.Command("tmux", "send-keys", "-t", sessionName+":0", command, "Enter")
		if err := sendCmd.Run(); err != nil {
			return fmt.Errorf("failed to send command to opencode: %w", err)
		}
		return nil
	}
